...
const BrandFormRows = ({ fields, formData, handleChange }) => {
  const rows = [];
  let currentRow = [];

const renderField = (field) => {
  const inputClass = field.fullWidth ? "input-group full-width" : "input-group";
  const isLogoField = field.name.toLowerCase().includes("logo") || field.name.toLowerCase().includes("image");
      ...
      {isLogoField && (
        <>
          ...
          {value && typeof value === "string" && (
            <img
              src={`https://myworkstatus.in/ecom/admin/images/brand/${value}`}
              alt="Preview"
              style={{
                marginTop: "10px",
                maxHeight: "100px",
